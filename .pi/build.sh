...
# docker buildx build --tag wizechain1 --file .pi/Dockerfile --platform linux/arm64 --cache-from wizechain1:cache --cache-to wizechain1:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar wizechain1
...
sudo bash -c "echo wizechain1 > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/wizechain1.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/wizechain1.img
